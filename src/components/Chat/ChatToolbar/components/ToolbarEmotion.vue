<template>
	<div class="emotion">
		...
		<div v-loading="loading" class="emotion-content">
			<emotion v-if="!emoticonList.length" ref="emotionRef" :padding="0" :show-categories="false" @emotion="emotion" />
...
export default {
  ...
  watch: {
    currentCategory(val) {
      if (this.$refs.emotionRef) {
        this.$refs.emotionRef.switchCategory(val);
      }
    }
  },
  created() {
